...
test_queries()
{
	...
	pass=true
	test_query qa "select * from qa order by pizza;"
	test_query qb "select * from qb order by cname, pizza;"
	test_query qc "select * from qc order by cname, pizza;"
	test_query qd "select * from qd order by pizza;"
	test_query qe "select * from qe order by rname1, rname2;"
	test_query qf "select * from qf order by pizza, rname;"
	if $pass; then
